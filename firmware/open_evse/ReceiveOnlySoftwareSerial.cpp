/*
ReceiveOnlySoftwareSerial.cpp (formerly NewSoftSerial.cpp) - 
...
uint8_t ReceiveOnlySoftwareSerial::_receive_buffer[_SS_MAX_RX_BUFF]; 
...
bool ReceiveOnlySoftwareSerial::stopListening()
{
  if (active_object == this)
  {
    setRxIntMsk(false);
...
// Constructor
//
ReceiveOnlySoftwareSerial::ReceiveOnlySoftwareSerial(int8_t receivePin, bool inverse_logic /* = false */) : 
...
void ReceiveOnlySoftwareSerial::setRX(int8_t rx)
{
  pinMode(rx, INPUT);
  if (!_inverse_logic)
...
uint16_t ReceiveOnlySoftwareSerial::subtract_cap(uint16_t num, uint16_t sub) {
  if (num > sub)
    return num - sub;
  else
    return 1;
...
void ReceiveOnlySoftwareSerial::setRxIntMsk(bool enable)
{
    if (enable)
